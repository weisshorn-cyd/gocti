...
type Feedback struct {
	graphql.Feedback `gocti:",squash"`
}

//go:embed default_properties/feedback_default_properties.txt
var feedbackDefaultProperties string

func (f Feedback) DefaultProperties() string {
	return feedbackDefaultProperties
}

// Implementing the [api.ListableEntity] interface.

//go:embed list_queries/feedback_list_query.txt
var feedbackListQueryString string

func (f Feedback) ListQueryString(customAttributes string) string {
	return fmt.Sprintf(
		feedbackListQueryString,
		customAttributes,
	)
}

func (f Feedback) ListResponseField() string { return "feedbacks" }

// Implementing the [api.ReadableEntity] interface.

func (f Feedback) ReadQueryString(customAttributes string) string {
	return fmt.Sprintf(
		`query ($id: String!) {
            feedback (id: $id) {%s}
        }`,
...
func (f Feedback) ReadResponseField() string { return "feedback" }

// Implementing the [api.CreatableEntity] interface.

func (f Feedback) CreateQueryString(customAttributes string) string {
	return fmt.Sprintf(
		`mutation ($input: FeedbackAddInput!) {
            feedbackAdd (input: $input) {%s}
        }`,
		customAttributes,
	)
}

func (f Feedback) CreateResponseField() string { return "feedbackAdd" }

// FeedbackAddInput represents a GraphQL INPUT_OBJECT
...
func (input FeedbackAddInput) Input() (map[string]any, error) {
...
func (f Feedback) DeleteQueryString() string {
	return `mutation ($id: ID!) {
                feedbackDelete (id: $id)
            }`
}

func (f Feedback) DeleteResponseField() string { return "feedbackDelete" }
