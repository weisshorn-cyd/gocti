// Code generated by '/tools/gocti_type_generator' for OpenCTI version 7.260326.0 - DO NOT EDIT.

package entity

import (
	"fmt"
	"time"

	"github.com/weisshorn-cyd/gocti/graphql"

	_ "embed"
)

type ThreatActorIndividual struct {
	graphql.ThreatActorIndividual `gocti:",squash"`
}

//go:embed default_properties/threat_actor_individual_default_properties.txt
var threatActorIndividualDefaultProperties string

func (t ThreatActorIndividual) DefaultProperties() string {
	return threatActorIndividualDefaultProperties
}

// Implementing the [api.ListableEntity] interface.

//go:embed list_queries/threat_actor_individual_list_query.txt
var threatActorIndividualListQueryString string

func (t ThreatActorIndividual) ListQueryString(customAttributes string) string {
	return fmt.Sprintf(
		threatActorIndividualListQueryString,
		customAttributes,
	)
}

func (t ThreatActorIndividual) ListResponseField() string { return "threatActorsIndividuals" }

// Implementing the [api.ReadableEntity] interface.

func (t ThreatActorIndividual) ReadQueryString(customAttributes string) string {
	return fmt.Sprintf(
		`query ($id: String!) {
            threatActorIndividual (id: $id) {%s}
        }`,
		customAttributes,
	)
}

func (t ThreatActorIndividual) ReadResponseField() string { return "threatActorIndividual" }

// Implementing the [api.CreatableEntity] interface.

func (t ThreatActorIndividual) CreateQueryString(customAttributes string) string {
	return fmt.Sprintf(
		`mutation ($input: ThreatActorIndividualAddInput!) {
            threatActorIndividualAdd (input: $input) {%s}
        }`,
		customAttributes,
	)
}

func (t ThreatActorIndividual) CreateResponseField() string { return "threatActorIndividualAdd" }

// ThreatActorIndividualAddInput represents a GraphQL INPUT_OBJECT
// Some fields from the OpenCTI schema may be missing
// (See the examples for ways to expand an existing type).
type ThreatActorIndividualAddInput struct {
	// ThreatActorIndividualAddInput
	StixID               string                 `gocti:"stix_id"               json:"stix_id,omitempty"`
	XOpenctiStixIDs      []string               `gocti:"x_opencti_stix_ids"    json:"x_opencti_stix_ids,omitempty"`
	Name                 string                 `gocti:"name"                  json:"name,omitempty"`
	Description          string                 `gocti:"description"           json:"description,omitempty"`
	Aliases              []string               `gocti:"aliases"               json:"aliases,omitempty"`
	ThreatActorTypes     []string               `gocti:"threat_actor_types"    json:"threat_actor_types,omitempty"`
	FirstSeen            *time.Time             `gocti:"first_seen"            json:"first_seen,omitempty"`
	LastSeen             *time.Time             `gocti:"last_seen"             json:"last_seen,omitempty"`
	Roles                []string               `gocti:"roles"                 json:"roles,omitempty"`
	Goals                []string               `gocti:"goals"                 json:"goals,omitempty"`
	Sophistication       string                 `gocti:"sophistication"        json:"sophistication,omitempty"`
	ResourceLevel        string                 `gocti:"resource_level"        json:"resource_level,omitempty"`
	PrimaryMotivation    string                 `gocti:"primary_motivation"    json:"primary_motivation,omitempty"`
	SecondaryMotivations []string               `gocti:"secondary_motivations" json:"secondary_motivations,omitempty"`
	PersonalMotivations  []string               `gocti:"personal_motivations"  json:"personal_motivations,omitempty"`
	DateOfBirth          *time.Time             `gocti:"date_of_birth"         json:"date_of_birth,omitempty"`
	Gender               string                 `gocti:"gender"                json:"gender,omitempty"`
	JobTitle             string                 `gocti:"job_title"             json:"job_title,omitempty"`
	MaritalStatus        string                 `gocti:"marital_status"        json:"marital_status,omitempty"`
	EyeColor             string                 `gocti:"eye_color"             json:"eye_color,omitempty"`
	HairColor            string                 `gocti:"hair_color"            json:"hair_color,omitempty"`
	Height               []graphql.MeasureInput `gocti:"height"                json:"height,omitempty"`
	Weight               []graphql.MeasureInput `gocti:"weight"                json:"weight,omitempty"`
	Confidence           int                    `gocti:"confidence"            json:"confidence,omitempty"`
	Revoked              bool                   `gocti:"revoked"               json:"revoked,omitempty"`
	Lang                 string                 `gocti:"lang"                  json:"lang,omitempty"`
	CreatedBy            string                 `gocti:"createdBy"             json:"createdBy,omitempty"`
	ObjectMarking        []string               `gocti:"objectMarking"         json:"objectMarking,omitempty"`
	ObjectOrganization   []string               `gocti:"objectOrganization"    json:"objectOrganization,omitempty"`
	ObjectAssignee       []string               `gocti:"objectAssignee"        json:"objectAssignee,omitempty"`
	ObjectLabel          []string               `gocti:"objectLabel"           json:"objectLabel,omitempty"`
	BornIn               string                 `gocti:"bornIn"                json:"bornIn,omitempty"`
	Ethnicity            string                 `gocti:"ethnicity"             json:"ethnicity,omitempty"`
	ExternalReferences   []string               `gocti:"externalReferences"    json:"externalReferences,omitempty"`
	Created              *time.Time             `gocti:"created"               json:"created,omitempty"`
	Modified             *time.Time             `gocti:"modified"              json:"modified,omitempty"`
	ClientMutationID     string                 `gocti:"clientMutationId"      json:"clientMutationId,omitempty"`
	XOpenctiWorkflowID   string                 `gocti:"x_opencti_workflow_id" json:"x_opencti_workflow_id,omitempty"`
	XOpenctiModifiedAt   *time.Time             `gocti:"x_opencti_modified_at" json:"x_opencti_modified_at,omitempty"`
	Update               bool                   `gocti:"update"                json:"update,omitempty"`
	File                 []byte                 `gocti:"file"                  json:"file,omitempty"`
	FileMarkings         []string               `gocti:"fileMarkings"          json:"fileMarkings,omitempty"`
	Files                [][]byte               `gocti:"files"                 json:"files,omitempty"`
	FilesMarkings        []string               `gocti:"filesMarkings"         json:"filesMarkings,omitempty"`
	NoTriggerImport      []bool                 `gocti:"noTriggerImport"       json:"noTriggerImport,omitempty"`
	Embedded             []bool                 `gocti:"embedded"              json:"embedded,omitempty"`
	UpsertOperations     []graphql.EditInput    `gocti:"upsertOperations"      json:"upsertOperations,omitempty"`
}

func (input ThreatActorIndividualAddInput) Input() (map[string]any, error) {
	return map[string]any{
		"input": input,
	}, nil
}

// Implementing the [api.DeletableEntity] interface.

func (t ThreatActorIndividual) DeleteQueryString() string {
	return `mutation ($id: ID!) {
                threatActorIndividualDelete (id: $id)
            }`
}

func (t ThreatActorIndividual) DeleteResponseField() string { return "threatActorIndividualDelete" }
